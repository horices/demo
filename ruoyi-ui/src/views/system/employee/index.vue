<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="部门名称" prop="deptName">
        <el-select v-model="queryParams.deptName" placeholder="请选择部门名称" clearable>
          <el-option
            v-for="dict in dict.type.employ_dept_name"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="学历" prop="empDegreeName">
        <el-select v-model="queryParams.empDegreeName" placeholder="请选择学历" clearable>
          <el-option
            v-for="dict in dict.type.employ_degree_name"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="employeeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="职员ID" align="center" prop="id" />
      <el-table-column label="职员姓名" align="center" prop="empName" />
      <el-table-column label="性别" align="center" prop="sex" />
      <el-table-column label="年龄" align="center" prop="age" />
      <el-table-column label="部门名称" align="center" prop="deptName">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.employ_dept_name" :value="scope.row.deptName"/>
        </template>
      </el-table-column>
      <el-table-column label="学历" align="center" prop="empDegreeName">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.employ_degree_name" :value="scope.row.empDegreeName"/>
        </template>
      </el-table-column>
...
export default {
  name: "Employee",
  dicts: ['employ_dept_name', 'employ_degree_name'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除职工编号为"' + ids + '"的数据项？').then(function() {
